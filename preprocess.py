...
def tokenize(texts, max_length, skip=0, attr=LOWER, merge=False, nlp=None,
             **kwargs):
    ...
    >>> arr.shape[1]
    10
    >>> w2i = {w: i for i, w in vocab.items()}
            ...
            with doc.retokenize() as retokenizer:
                for phrase in doc.noun_chunks:
                    ...
                    if len(phrase) > 1:
                        ...
                        retokenizer.merge(phrase)
                        # retokenizer.merge([phrase.root.tag_, phrase.text,
                        #              phrase.root.ent_type_])
                    # Iterate over named entities
                    for ent in doc.ents:
                        if len(ent) > 1:
                            # Merge them into single tokens
                            retokenizer.merge(ent.root.tag_, ent.text, ent.label_)
                            # retokenizer.merge([ent.root.tag_, ent.text, ent.label_])
    ...
    vocab = {v: nlp.vocab[v].lower_ for v in uniques}
    vocab[skip] = '<SKIP>'
    return data, vocab
